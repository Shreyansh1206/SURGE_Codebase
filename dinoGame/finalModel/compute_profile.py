...
from dino_env import FEATURES_PER_FRAME, N_ACTIONS, OBS_DIM
from ppo_agent import PPO, ActorCritic

FRAMES_PER_STEP = 4
GAME_HZ = 60.0
DECISIONS_PER_SEC = GAME_HZ / FRAMES_PER_STEP
...
def analytical_forward_macs(obs_dim: int, n_actions: int, hidden: int = 128):
    ...
    return {
        ...
        "trunk_mac": int(trunk),
        "activation_ops_estimate": int(hidden * 2),
        "softmax_argmax_ops_estimate": int(n_actions * 4 + 2),
    }
...
def measure_sustained_resources(body_fn, sampler, inner_iters: int = 5000):
    ...
    return {
        ...
        "inferences_per_second": (
            round(inner_iters / wall_sec, 2) if wall_sec > 0 else 0.0
        ),
    }
...
def pack_paced_profile(paced_data: dict):
    ...
    for key in (
        ...
        "session_cpu_seconds",
        "session_cpu_percent",
        "achieved_decisions_per_second",
        "model_duty_cycle_percent",
    ):
...
def build_report(data: dict) -> str:
    ...
    m = (
        paced["inference_latency_ms"]
        if paced
        else inf.get("argmax_deploy", {}).get("latency_ms", {})
    )
    ...
    if paced:
        ...
        if "cpu_percent" in paced:
            lines.append(
                f"| CPU % per tick (mean) | {paced['cpu_percent']['mean']:.2f}% |"
            )
...
def main():
    ...
    parser.add_argument("--ckpt", type=str, default="checkpoints/best_duck.pt")
    parser.add_argument(
        "--runs", type=int, default=25, help="Timed iterations per path (after warmup)."
    )
    ...
    if sampler is None:
        print(
            "[compute_profile] psutil not installed — CPU/RAM per-run metrics skipped. "
    ...
    if args.paced:
        print(
            f"[compute_profile] Paced simulation @ {args.decision_hz:.1f} Hz "
    ...
    if not args.paced or args.also_max_speed:
        run_forward = time_forward_only(
            net, obs_t, device, args.warmup, args.runs, sampler
        )
        run_argmax = time_argmax_deploy(
            net, obs_t, device, args.warmup, args.runs, sampler
        )
    ...
    if args.batch_size > 1 and (not args.paced or args.also_max_speed):
        ...
        run_batch = time_batch_forward(
            net, obs_b, device, args.warmup, args.runs, sampler
        )
        batch_results = {
    ...
    deploy = {
        ...
        "gflops_per_game_second": round(
            macs_per_deploy * DECISIONS_PER_SEC * 2 / 1e9, 6
        ),
    ...
    result = {
        ...
        "methodology": {
            ...
            "mode": (
                "paced_game"
                if args.paced and not args.also_max_speed
                else ("paced_game+max_speed" if args.paced else "max_speed")
            ),
            "decision_hz": args.decision_hz if args.paced else None,
            "description": (
                f"{args.runs} ticks at {args.decision_hz:.1f} Hz (paced game simulation)"
                if args.paced
                else f"{args.runs} timed iterations after {args.warmup} warmup passes; "
                "perf_counter wall time including host/device sync on CUDA."
            ),
            "resource_sampling": (
                "psutil process CPU% and RSS per timed iteration"
                if sampler
        ...
        "model": {
            ...
            "process_rss_mb_before": (
                round(mem_before, 3) if mem_before is not None else None
            ),
            "process_rss_mb_after": (
                round(mem_after, 3) if mem_after is not None else None
            ),
        },
        "analytical": {
            **macs_detail,
    ...
    if paced_packed is not None:
        ...
        print(
            f"  Mode           : paced @ {args.decision_hz:.1f} Hz ({args.runs} ticks)"
        )
        print(f"  Inference med  : {inf['median']:.4f} ms  mean {inf['mean']:.4f} ms")
        print(
            f"  Slot period med: {slot['median']:.2f} ms  (target {paced_packed['period_ms_target']:.2f})"
        )
        print(
            f"  Model duty     : {paced_packed.get('model_duty_cycle_percent', 0):.2f}% of slot"
        )
        ...
        if "cpu_percent" in paced_packed:
            ...
            print(
                f"  CPU % per-tick : mean {cpu['mean']:.2f}%  median {cpu['median']:.2f}%"
            )
            print(
                f"  RAM RSS        : mean {ram['mean']:.2f} MB  median {ram['median']:.2f} MB"
            )
        print(
            f"  Achieved rate  : {paced_packed.get('achieved_decisions_per_second', 0):.2f} Hz"
        )
    if "argmax_deploy" in inference:
        ad = inference["argmax_deploy"]["latency_ms"]
        print(f"  [max-speed] latency mean : {ad['mean']:.4f} ms")
        print(
            f"  [max-speed] throughput   : "
            f"{inference['argmax_deploy']['throughput_per_sec']['mean']:.1f} inf/s"
        )
    print(f"  @ 15 Hz MACs/s : {deploy['macs_per_game_second']:,}")
    if sys_mem:
        print(
            f"  System RAM     : {sys_mem['system_ram_percent']:.1f}% of "
            f"{sys_mem['system_ram_total_mb']:.0f} MB"
        )
    burst = resources.get("sustained_argmax_burst")
    if burst:
        print(
            f"  CPU % (burst)  : {burst['cpu_percent_of_one_core_process_time']:.2f}%"
        )
    print("-" * 60)
