...
def main():
    ...
    ppo.net.eval()
    print(
        f"[infer] loaded {args.ckpt}  |  mode={'sample' if args.sample else 'argmax'}  "
    ...
    try:
        for ep in range(1, args.episodes + 1):
            ...
            print(
                f"ep {ep:3d} | score {score:5d} (best {best_so_far:5d}) | "
        ...
        if scores:
            print(f"\n=== {len(scores)} episodes ===")
            print(
                f"  score   mean={np.mean(scores):6.1f}  max={np.max(scores):5d}  "
                f"min={np.min(scores):5d}  median={int(np.median(scores)):5d}"
            )
            print(
                f"  return  mean={np.mean(returns):+7.2f}  max={np.max(returns):+7.2f}"
            )
        env.close()
