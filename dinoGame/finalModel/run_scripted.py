...
import numpy as np
from dino_env import N_ACTIONS, DinoEnv
from scripted_teacher import new_memory, scripted_action
...
def main():
    ...
    args = p.parse_args()
    print(f"[scripted] episodes={args.episodes} curriculum_prob={args.curriculum_prob}")
    env = DinoEnv(
    ...
    print("\n=== Summary ===")
    print(
        f"  scores: mean={np.mean(scores):.1f}  std={np.std(scores):.1f}  "
        f"median={np.median(scores):.1f}  max={max(scores)}  min={min(scores)}"
    )
    print(
        f"  actions: noop={actions[0]} jump={actions[1]} duck={actions[2]}  "
