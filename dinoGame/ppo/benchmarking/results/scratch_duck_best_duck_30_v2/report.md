...
| N episodes | 30 |
| Mean | 2339.57 |
| Std. deviation | 1664.24 |
| Std. error of mean | 303.847 |
| 95% CI (lower) | 1744.03 |
| 95% CI (upper) | 2935.11 |
| Median | 1994.5 |
| IQR (P25 – P75) | 1278.5 |
...
| Median steps | 1620.0 |
| Max steps | 4646 |
| Mean shaped return | 10.874 |
| Std shaped return | 14.788 |
| Max shaped return | 50.250 |
| Mean wall-time (s) | 148.08 |
| Max wall-time (s) | 381.56 |
| Mean final speed | 12.171 |
...
**Deterministic — global action entropy:** 0.6675 nats  
**Deterministic — mean per-episode action entropy:** 0.6092 nats  
...
|:------------|---------:|--:|
| CACTUS_LARGE | 15 | 50.0% |
| CACTUS_SMALL | 10 | 33.3% |
| PTERODACTYL | 5 | 16.7% |

**Pterodactyl-death rate:** 16.7%  
...
The deterministic policy achieves a mean score of **2339.57 ± 1664.24** (median 1994.5, max 6873) over 30 episodes. The coefficient of variation (CV = 0.711) reflects environmental stochasticity — obstacle types, gaps, and the speed ramp-up are non-deterministic from the agent's perspective, so score variance is partly irreducible. Survival rates of 96.7% at score ≥200 and 96.7% at score ≥300 suggest the policy has internalised basic obstacle-avoidance behaviour and can maintain game-play through the early speed-increase phases.

### 5.2 Action Selection Behaviour

Under deterministic inference the agent selects **jump** in 14.3% of steps and **duck** in 7.6% of steps. Jump is the primary survival action and its frequency reflects how densely obstacles appear relative to the nominal decision horizon. Low duck frequency is expected: pterodactyls only appear at higher speeds and ducking is rarely the correct action in the early-to-mid score range captured by most evaluation episodes. The global action entropy of 0.6675 nats (vs. maximum 1.0986 nats) indicates a concentrated, relatively deterministic behavioural profile.

### 5.3 Critic Value Estimates

The critic outputs values in the range [-5.71, -1.51] with a mean of -4.260. The predominantly negative value range is consistent with the reward structure: death produces a −10 penalty that dominates shaped rewards, so the critic learns to predict a slightly negative long-run return at most states, scaling toward zero as the agent survives longer episodes.
