...
**Generated:** 20260531_220433  
**Checkpoint:** `checkpoints_duck_v13/best_duck.pt`  
...
| N episodes | 25 |
| Mean | 565.80 |
| Std. deviation | 154.96 |
| Std. error of mean | 30.992 |
| 95% CI (lower) | 505.06 |
| 95% CI (upper) | 626.54 |
| Median | 548.0 |
| IQR (P25 – P75) | 129.0 |
...
| Median steps | 608.0 |
| Max steps | 1018 |
| Mean shaped return | -4.781 |
| Std shaped return | 1.452 |
| Max shaped return | 0.110 |
| Mean wall-time (s) | 49.42 |
| Max wall-time (s) | 84.27 |
| Mean final speed | 9.049 |
| Max final speed | 11.084 |
...
**Deterministic — global action entropy:** 0.2552 nats  
**Deterministic — mean per-episode action entropy:** 0.2492 nats  
...
The deterministic policy achieves a mean score of **565.80 ± 154.96** (median 548.0, max 1084) over 25 episodes. The coefficient of variation (CV = 0.274) reflects environmental stochasticity — obstacle types, gaps, and the speed ramp-up are non-deterministic from the agent's perspective, so score variance is partly irreducible. Survival rates of 100.0% at score ≥200 and 96.0% at score ≥300 suggest the policy has internalised basic obstacle-avoidance behaviour and can maintain game-play through the early speed-increase phases.

### 5.2 Action Selection Behaviour

Under deterministic inference the agent selects **jump** in 6.7% of steps and **duck** in 0.1% of steps. Jump is the primary survival action and its frequency reflects how densely obstacles appear relative to the nominal decision horizon. Low duck frequency is expected: pterodactyls only appear at higher speeds and ducking is rarely the correct action in the early-to-mid score range captured by most evaluation episodes. The global action entropy of 0.2552 nats (vs. maximum 1.0986 nats) indicates a concentrated, relatively deterministic behavioural profile.

### 5.3 Critic Value Estimates

The critic outputs values in the range [-6.65, -1.15] with a mean of -3.249. The predominantly negative value range is consistent with the reward structure: death produces a −10 penalty that dominates shaped rewards, so the critic learns to predict a slightly negative long-run return at most states, scaling toward zero as the agent survives longer episodes.
