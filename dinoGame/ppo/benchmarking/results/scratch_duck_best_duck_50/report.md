...
| N episodes | 50 |
| Mean | 879.48 |
| Std. deviation | 801.79 |
| Std. error of mean | 113.390 |
| 95% CI (lower) | 657.24 |
| 95% CI (upper) | 1101.72 |
| Median | 688.5 |
| IQR (P25 – P75) | 1493.5 |
...
| Median steps | 611.5 |
| Max steps | 2228 |
| Mean shaped return | -1.610 |
| Std shaped return | 7.602 |
| Max shaped return | 17.140 |
| Mean wall-time (s) | 63.85 |
| Max wall-time (s) | 180.37 |
| Mean final speed | 9.669 |
...
**Deterministic — global action entropy:** 0.5823 nats  
**Deterministic — mean per-episode action entropy:** 0.4217 nats  
...
|:------------|---------:|--:|
| CACTUS_LARGE | 28 | 56.0% |
| CACTUS_SMALL | 19 | 38.0% |
| PTERODACTYL | 3 | 6.0% |

**Pterodactyl-death rate:** 6.0%  
...
The deterministic policy achieves a mean score of **879.48 ± 801.79** (median 688.5, max 2912) over 50 episodes. The coefficient of variation (CV = 0.912) reflects environmental stochasticity — obstacle types, gaps, and the speed ramp-up are non-deterministic from the agent's perspective, so score variance is partly irreducible. Survival rates of 68.0% at score ≥200 and 58.0% at score ≥300 suggest the policy has internalised basic obstacle-avoidance behaviour and can maintain game-play through the early speed-increase phases.

### 5.2 Action Selection Behaviour

Under deterministic inference the agent selects **jump** in 11.8% of steps and **duck** in 6.0% of steps. Jump is the primary survival action and its frequency reflects how densely obstacles appear relative to the nominal decision horizon. Low duck frequency is expected: pterodactyls only appear at higher speeds and ducking is rarely the correct action in the early-to-mid score range captured by most evaluation episodes. The global action entropy of 0.5823 nats (vs. maximum 1.0986 nats) indicates a concentrated, relatively deterministic behavioural profile.

### 5.3 Critic Value Estimates

The critic outputs values in the range [-5.67, -1.51] with a mean of -3.837. The predominantly negative value range is consistent with the reward structure: death produces a −10 penalty that dominates shaped rewards, so the critic learns to predict a slightly negative long-run return at most states, scaling toward zero as the agent survives longer episodes.
