"""
benchmark.py — Research-grade benchmarking for the Chrome Dino PPO agent.

Runs N episodes with the trained policy, collects per-episode and per-step
metrics, computes comprehensive statistics (mean, std, median, IQR, CI,
percentiles, survival rates, action entropy, value estimates), and writes:

    benchmarking/results/run_<TIMESTAMP>/
        metrics.json        — full per-episode data + aggregate statistics
        report.md           — research-paper-style summary report
        figures/
            ...
            training_curve.png   (only if ../checkpoints/train_log.jsonl exists)

Run from dinoGame/ppo/:
    python benchmarking/benchmark.py
    python benchmarking/benchmark.py --episodes 30 --both-modes
    python benchmarking/benchmark.py --ckpt checkpoints/best.pt --episodes 50
    python benchmarking/benchmark.py --headless --step-pause 0 --episodes 20

The script never modifies any source file outside its own results/ folder.
"""
...
# Ensure progress lines appear immediately (conda run / IDE terminals often block-buffer).
print = functools.partial(print, flush=True)

import numpy as np
import torch

# ── Path setup ── allow imports from dinoGame/ppo/
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from dino_env import DinoEnv, OBS_DIM, N_ACTIONS, FEATURES_PER_FRAME   # noqa: E402
...
def _ci_normal(data, confidence=0.95):
    """Normal-approximation CI — fallback when scipy is absent."""
    n = len(data)
    if n < 2:
        m = float(np.mean(data)) if n else float("nan")
        return m, m
...
def descriptive_stats(arr):
    """Return a rich dict of descriptive statistics for a 1-D array."""
...
def action_entropy(counts):
    """Shannon entropy of action distribution in nats."""
    total = counts.sum()
    if total == 0:
        return 0.0
    p = counts / total
...
class EpisodeRecord:
    __slots__ = (
        "episode", "score", "ep_return", "steps", "wall_time",
        "action_counts", "value_estimates", "final_speed", "mode",
        "death_cause",
    )

    def __init__(self, episode, score, ep_return, steps, wall_time,
                 action_counts, value_estimates, final_speed, mode,
                 death_cause=""):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "action_counts": {
                "noop": int(ac[0]), "jump": int(ac[1]), "duck": int(ac[2]),
            },
            "action_fracs": {
...
def _run_one_episode(env, ppo, ep, n_episodes, argmax, verbose=True):
    """Run a single evaluation episode; return EpisodeRecord."""
    if verbose:
    ...
    while not done:
        ...
        with torch.no_grad():
            ...
            action = int(np.argmax(probs)) if argmax else int(
    ...
    return EpisodeRecord(
        episode=ep, score=score, ep_return=ep_ret, steps=steps,
        wall_time=wall, action_counts=action_counts,
        value_estimates=value_estimates, final_speed=final_speed,
...
def warmup_env(env, n_steps=90):
    """Let Chrome/game settle after launch — first counted ep is often bogus without this."""
    obs = env.reset()
...
def _episode_needs_fresh_browser(rec, bad_score=300, bad_steps=100):
    """Heuristic for Selenium/Chrome drift (instant deaths at low speed)."""
    return rec.score < bad_score or rec.steps < bad_steps


def run_episodes(env, ppo, n_episodes, mode, verbose=True,
                 browser_restart_every=0, make_env=None,
                 adaptive_restart_streak=2):
    """
    Execute n_episodes with the given policy.

    Parameters
    ----------
    mode : "deterministic" | "stochastic"
        deterministic → argmax; stochastic → categorical sample.
    browser_restart_every : int
        Quit and relaunch Chrome every N completed episodes (0 = disabled).
        Requires make_env when > 0.
    make_env : callable
        Factory returning a fresh DinoEnv (used for periodic restarts).

    Returns
    -------
    list[EpisodeRecord]
        Partial results are returned if the run is interrupted with Ctrl+C.
    """
    records   = []
    best      = -1
    ...
    def _maybe_restart_browser(ep_just_finished, rec):
        ...
        if bad_streak >= streak_limit:
            ...
            return
        # Long episodes correlate with the next several instant-death runs.
        if rec.steps >= 1400:
            _restart_browser(
...
def aggregate(records):
    ...
    per_ep_entropy = [
        action_entropy(r.action_counts) for r in records
    ]

    # Death-cause breakdown (requires the env to tag info["death_obstacle"];
    # older runs without it fall back to "unknown").
    death_causes = {}
    for r in records:
...
def make_figures(records_det, records_sto, out_dir, train_log_path=None):
    ...
    ax.hist(scores_det, bins=bins, alpha=0.72, label="Deterministic",
            color=_PALETTE["noop"], edgecolor="white", linewidth=0.4)
    if scores_sto:
        ax.hist(scores_sto, bins=bins, alpha=0.60, label="Stochastic",
                color=_PALETTE["jump"], edgecolor="white", linewidth=0.4)
    ax.axvline(np.mean(scores_det), color=_DARK["noop"], linestyle="--",
               linewidth=1.8, label=f"Det. μ = {np.mean(scores_det):.1f}")
    if scores_sto:
        ax.axvline(np.mean(scores_sto), color=_DARK["jump"], linestyle="--",
                   linewidth=1.8, label=f"Sto. μ = {np.mean(scores_sto):.1f}")
    ...
    # ── 2. Empirical CDF ───────────────────────────────────────────────────
    fig, ax = plt.subplots(figsize=(9, 5))
    for label, sc, col in [("Deterministic", scores_det, _PALETTE["noop"]),
        ...
        ax.axvline(float(np.median(s)), color=col, linestyle=":",
    ...
    # ── 3. Score per episode (with rolling mean) ───────────────────────────
    fig, ax = plt.subplots(figsize=(11, 5))
    ep_det = [r.episode for r in records_det]
    ax.plot(ep_det, scores_det, alpha=0.45, color=_PALETTE["noop"],
            linewidth=0.9, zorder=1)
    ax.scatter(ep_det, scores_det, color=_PALETTE["noop"], s=18,
               alpha=0.80, zorder=2, label="Deterministic")
    w = max(3, len(scores_det) // 6)
    if len(scores_det) >= w:
        roll = np.convolve(scores_det, np.ones(w) / w, mode="valid")
        ax.plot(ep_det[w - 1:], roll, color=_DARK["noop"], linewidth=2.0,
                zorder=3, label=f"Rolling mean (w={w})")
    if scores_sto:
        ep_sto = [r.episode for r in records_sto]
        ax.plot(ep_sto, scores_sto, alpha=0.35, color=_PALETTE["jump"],
                linewidth=0.9, zorder=1)
        ax.scatter(ep_sto, scores_sto, color=_PALETTE["jump"], s=18,
                   alpha=0.70, zorder=2, label="Stochastic")
    ...
    for ax, (label, recs) in zip(axes, datasets):
        ...
        wedges, texts, autotexts = ax.pie(
            fracs, labels=ACTION_NAMES, colors=wedge_cols,
    ...
    # ── 5. Value function distribution ─────────────────────────────────────
    vals_det = [v for r in records_det for v in r.value_estimates]
    if vals_det:
        fig, ax = plt.subplots(figsize=(9, 5))
        ax.hist(vals_det, bins=60, color=_PALETTE["noop"], alpha=0.80,
                edgecolor="white", linewidth=0.3)
        ax.axvline(float(np.mean(vals_det)), color=_DARK["noop"], linestyle="--",
                   linewidth=1.8, label=f"Mean = {np.mean(vals_det):.3f}")
        ax.axvline(float(np.median(vals_det)), color=_DARK["noop"], linestyle=":",
                   linewidth=1.4, label=f"Median = {np.median(vals_det):.3f}")
        ax.set_xlabel("V(s)  —  Critic Output", fontsize=12)
        ax.set_ylabel("Count", fontsize=12)
        ax.set_title("Value Function Distribution (Deterministic Mode)",
    ...
    # ── 6. Training curve (if log exists) ──────────────────────────────────
    if train_log_path and os.path.exists(train_log_path):
        ...
        if rows:
            ...
            axes[0, 0].plot(updates, mean_sc, color=_PALETTE["noop"],
                            linewidth=1.6, label="Mean score")
            axes[0, 0].plot(updates, max_sc,  color=_PALETTE["jump"],
                            linewidth=1.0, alpha=0.7, label="Max score")
...
def write_report(
    out_dir, ckpt_path, n_det, n_sto,
    det_agg, sto_agg,
    records_det, records_sto,
    run_ts, train_log_rows=None, model_params=None,
):
    lines = []
    A = lines.append
    ...
    A(f"**scipy available:** {'Yes' if _HAS_SCIPY else 'No (CI via normal approximation)'}  ")
    A("")
    A("---")
    A("")
    ...
    A("| State features | dino_y, jumping, ducking, speed, o1_dx, o1_y, o1_w, o1_h, o2_dx, o2_y, o2_w, o2_h |")
    ...
    A("")
    A("> Percentage of episodes in which the agent reached or exceeded each score threshold.")
    A("")
    ...
    A(f"**Deterministic — mean per-episode action entropy:** {_fmt(det_ment, 4)} nats  ")
    if sto_agg_ok:
        ...
        A(f"**Stochastic — mean per-episode action entropy:** {_fmt(sto_ment, 4)} nats  ")
    ...
    # ── 3.3 Death-cause breakdown ─────────────────────────────────────────────
    dc_d = det_agg.get("death_causes", {})
    if dc_d:
        A("### 3.3 Death-Cause Breakdown (Deterministic Mode)")
        A("")
        A("> Obstacle type the agent collided with at episode end. "
    ...
    if train_log_rows:
        ...
        A("")
        A(f"*Based on `checkpoints/train_log.jsonl` — "
        ...
        if valid:
            ...
            A("|:------|--------:|-----------:|-----------:|-------------:|---------:|")
            for start, end, phase in [(1, a, "Early"), (a+1, b, "Mid"),
                                      (b+1, n, "Late")]:
                sub = [r for r in train_log_rows
                ...
                mean_ln_vals = [r["mean_len"] for r in sub
                ...
                A(_md_table_row([phase, f"{start}–{end}", peak_sc,
                                  f"{mean_sc:.1f}", f"{mean_ln:.1f}", f"{mean_sp:.1f}"]))
            A("")
    ...
    A("---")
    A("")
    A("*Report generated automatically by `benchmarking/benchmark.py` — "
...
def main():
    ...
    parser.add_argument("--episodes", type=int, default=20,
                        help="Deterministic evaluation episodes.")
    ...
    parser.add_argument("--browser-restart-every", type=int, default=5,
                        help="Quit and relaunch Chrome every N episodes (0=never).")
    parser.add_argument("--adaptive-restart-streak", type=int, default=2,
                        help="Restart after this many consecutive weak episodes "
                             "(score<300 or steps<100); 0=disable.")
    parser.add_argument("--seed", type=int, default=42,
                        help="NumPy / PyTorch random seed.")
    parser.add_argument("--out-dir", type=str, default=None,
                        help="Output directory (default: benchmarking/results/run_<TS>).")
    ...
    if args.out_dir:
        ...
        out_dir = os.path.join(
            os.path.dirname(__file__), "results", f"run_{run_ts}"
        )
    ...
    print(f"[benchmark] Loaded {ckpt_path}  device={ppo.device}  params={model_params:,}")
    ...
    if adapt_streak > 0:
        print(f"[benchmark] Adaptive restart after {adapt_streak} weak episode(s) in a row")
    ...
    try:
        ...
        records_det = run_episodes(
            env_holder[0], ppo, n_det, mode="deterministic", verbose=True,
            browser_restart_every=restart_every, make_env=relaunch_env,
        ...
        if n_sto > 0:
            ...
            records_sto = run_episodes(
                env_holder[0], ppo, n_sto, mode="stochastic", verbose=True,
                browser_restart_every=restart_every, make_env=relaunch_env,
                adaptive_restart_streak=adapt_streak,
            )
    ...
    # ── Figures ──────────────────────────────────────────────────────────────
    make_figures(
        records_det, records_sto, out_dir,
    ...
    print(f"  Score 95% CI      : [{_fmt(ss.get('ci95_lo'))}, {_fmt(ss.get('ci95_hi'))}]")
    ...
    if records_sto:
        ...
        print(f"  Sto. mean +/- std : {_fmt(ss_s.get('mean'))} +/- {_fmt(ss_s.get('std'))}")
    print("-" * 58)
