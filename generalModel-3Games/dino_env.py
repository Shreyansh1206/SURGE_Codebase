...
class DinoEnv:
    ...
    def __init__(
        ...
        if render:
            ...
            # One game frame per RL step when watching — avoids 4-frame visual jumps.
    ...
    def reset(self):
        ...
        if self.render:
            self.engine._maybe_render(throttle=False)
    ...
    def step(self, action: int):
        ...
        for frame_i in range(self.frames_per_step):
            state, done, info = self.engine.step(
                action, draw=self.render, throttle=self.render
            )
            self.frame_buffer.append(self._featurize(state))
